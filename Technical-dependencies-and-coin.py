...
facts_list = ['Илон Маск также выступает за регулирование социальных сетей и защиту личных данных пользователей. Он утверждает, что социальные сети собирают огромное количество информации о нас, которую потом можно использовать для манипулирования нашими мыслями и поведением.',
...
coin_list = ['Решка!', 'Орёл!', 'Ого! Монетка встала на ребро!']

@app.route("/")
def hello_world():
    return '<p>Здесь ты узнаешь случайный факт о технологических зависимостях</p> <a href="/random_fact">Посмотреть случайный факт!</a> <a href="/random_coin">Бросить монетку</a>'
